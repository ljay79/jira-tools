...
var TestTable = {
  ...
  /*
   * Range names: - Can contain only letters, numbers, and underscores. - Can't
   * start with a number, or the words "true" or "false." - Can't contain any
   * spaces or punctuation. - Must be 1–250 characters. - Can't be in either A1
   * or R1C1 syntax. For example, you might get an error if you give your range
   * a name like "A1:B2" or "R1C1:R2C2."
   */
...
function testDefineRange() {
  ...
  console.log('rangeCheckName: %s === TestTable.rangeA1', rangeCheckName,
      TestTable.rangeA1);
}
...
function onEditTableMeta(e) {
  ...
  case 'INSERT_ROW':
    ...
    var tableHeader = getTicketSheet().getRange(
        rangeCoord.r + TestTable.headerRowOffset, rangeCoord.c, 1,
        (rangeCheck.getLastColumn() - rangeCoord.c + 1));
    // header changed?
    if (tableHeader.getValues()[0].join('|') !== TestTable.headerValues
        .join('|')) {
      console.log('Compare header: %s !== %s', tableHeader.getValues()[0]
          .join('|'), TestTable.headerValues.join('|'));
      console.log('New header values: %s', tableHeader.getValues());

      /*
       * @TODO: logic needed to check new header values - parse and check if new
       * value is a supported Jira field - yes: update meta - no: flag column to
       * be skipped - header moved inside table range (relative offset update?) /
       * not 2nd row anymore? allowed at all?
       */
    ...
    if (rangeCheck.getA1Notation() === TestTable.rangeA1) {
      ...
      /*
       * !! check if new change is affecting IssueTable functionality If YES:
       * prompt user with warning and undo or accept with IssueTable beeing
       * converted to static sheet table (remove trigger) If NO: simply update
       * TableMeta if necessary
       */
...
/**
 * @file Contains class used reflect a Jira IssueTable's meta data for google
 *       sheet tables.
 */

/**
 * Creates new IssueTable_ instance to reflect the meta data of a IssueTable in
 * google sheets.
 * 
 * @param {object}
 *          data Optional JSON representation of previously stored IssueTable
 *          data object.
 * @Constructor
 */
function IssueTable_(data) {
  ...
  this.data = {
     ...
     * Can't start with a number, or the words "true" or "false." - Can't
     * contain any spaces or punctuation. - Must be 1–250 characters. - Can't be
     * in either A1 or R1C1 syntax. For example, you might get an error if you
     * give your range a name like "A1:B2" or "R1C1:R2C2."
     */
    rangeName : 'table1_6_02713257E8',
    rangeA1 : 'B3:D6',
    headerRowOffset : 1,
  ...
  /**
   * Takes stringified JSON to parse into JSON object and use for initialize a
   * IssueTable object.
   * 
   * @param {string}
   *          json The JSON string to parse and load into a new IssueTable
   *          instance
   * @return {IssueTable_} A new instance of IssueTable_ with all data from
   *         [json] load into.
   */
