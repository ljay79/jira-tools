...
/**
 * Creates a new IssueTableIndex_ instance, which is used to persist IssueTables
...
function IssueTableIndex_() {
   ...
   * 
   * @param {IssueTable_}
   *          IssueTable The IssueTable object to store in script storage
   ...
   * 
   * @param {string}
   *          tableId Table Id to fetch
   * @param {sheetId}
   *          sheetId Optional: Sheet Id to fetch table for. Default: current
   *          active sheets id
   * @return {IssueTable_} Returns instance of IssueTable with loaded data if
   *         found, or empty instance if not
   */
   ...
   * 
   * @param {string}
   *          sheetId The sheet's sanitized id
   * @param {string}
