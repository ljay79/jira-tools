...
function unifyIssueAttrib(attrib, data) {
  ...
  if ( attrib.substring(0, 12) == 'customfield_' ) {
    ...
    // custom epic
    var epicField = Storage.getValue('jst_epic');
    ...
    if (customFields.hasOwnProperty(attrib)) {
      ...
      switch(format) {
        ...
        case 'user':
          resp = {
            value: (Storage.getValue('dspuseras_name') == 1 ? data.fields[attrib].displayName : data.fields[attrib].name) || 'Unknown',
            avatarUrls: data.fields[attrib].avatarUrls['24x24'] || ''
          };
          break;
        case 'array|user':
          resp.value = data.fields[attrib].map(function(el){
            return ((Storage.getValue('dspuseras_name') == 1 ? el.displayName : el.name) || 'Unknown');
          }).join(', ');
          break;
        case 'group':
  ...
  switch(attrib) {
    ...
    case 'aggregatetimeestimate':
      resp = {
        value: (Storage.getValue('dspdurationas') == "w") ? formatTimeDiff(parseInt(data.fields[attrib]) || 0) : parseInt(data.fields[attrib]) || 0
      };
      break;
    case 'project':
      resp = {
