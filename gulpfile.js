...
/*
Copys source code into the dist/build folder
Comments out code which is required for the code base to run locally in Node and would error in GAS
*/
...
/*
Undoes the clasp conversion of JS and GS file extensions
Also uncomments code which is required for the code base to run locally in Node
*/
gulp.task('un-google', function (done) {
  ...
  /*
  Diffs the code in dist/pull which has been downloaded from GAS with the current source
  */
gulp.task('diff-pulled-code', function (done) {
  ...
  /*
  Copys the changed code in dist/pull which has been downloaded from GAS into the src folder
  */
...
/*
Pulls GAS source code into dist/pull and compares it visually so it can be copied over into src if required
*/
